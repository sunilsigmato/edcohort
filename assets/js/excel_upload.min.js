...
$(function(){
    ...
    $('body').on('change', "input.file-activity", function() {
        ...
        S_Data =0;
                    ...
                    $.ajax({
                        url: base_url+"admin_brand/import_ajax_save",
                        data: formData,
                        processData: false,
                        contentType: false,
                        type: 'POST',
    ...
    function call_processing_area(yx,res){
        ...
        $.ajax({
                        url: base_url+"admin_brand/read_excel_values",
                        data: {ajax:1,
    ...
    function validate_and_processs_to_db(yx,data){
        ...
        yx.find('.myprogress').css('width', '100%');
         ...
         $.ajax({
                        url: Settings.base_url+"delivery/uploadPincode/push-excel-values-db",
                        data:data,
                        type: 'POST',
    ...
    function excel_html_handler(data){
        ...
        if(data.employee_duplicates.total > 0){
            $(".e-b-p-area").show();
            $(".e-b-p span").html(data.employee_duplicates.total);
            insert_values_to_db($(".e-b-p-area").find(".e-b-p-t"),data.employee_duplicates.data);
        }
    ...
    function insert_values_to_db($t,data){
            $.each( data, function( i, val ) {
                ...
                $rowdata= "<tr><td>"+val.row_index+"</td><td>"+val[0]+
                "</td><td>"+val.msg+"</td></tr>";
